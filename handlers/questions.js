...
export default async function handler(req, res) {
  ...
  if (req.method === 'DELETE') {
    ...
    try {
      ...
      await client.query(
        `DELETE FROM yks2_question_logs
         WHERE id = $1
           AND user_id = $2`,
        [
          id,
  ...
  if (
    ...
    return res.status(400).json({
      error:
        'Alanına uygun bir sınav ve ders seç.'
    });
  }
  ...
  if (sourceId) {
    ...
    if (!own.rows.length) {
      return res.status(400).json({
        error:
          'Seçilen kaynak bu hesaba ait değil.'
      });
    }

    if (
      own.rows[0].exam !== exam ||
      own.rows[0].subject !==
        subject
    ) {
      return res.status(400).json({
        error:
          'Seçilen kaynak, soru kaydındaki sınav ve dersle eşleşmiyor.'
      });
    }
  }
  ...
  try {
    ...
    const result =
      await client.query(
        `INSERT INTO yks2_question_logs(
          user_id,
          exam,
          subject,
          topic,
