...
/*
  AI Pro maliyet güvenliği.

  Bunlar AI'ın çalışma şeklini değiştirmez.
  Daha sonra aylık kullanıcı bütçesini güvenli
  biçimde kontrol etmek için kullanılacak.
*/
...
async function getMonthlyAiCostUsd(userId) {
  if (!userId) {
    return 0;
  }
  ...
  return Number(
    result.rows?.[0]?.total_cost_usd || 0
  );
}
/*
...
export async function geminiText({
  ...
  const monthlyCostUsd =
    await getMonthlyAiCostUsd(userId);

  if (
    monthlyCostUsd +
      AI_COST_SAFETY.reservePerRequestUsd >
