...
<button id="logoutAllDevicesButton" class="btn danger" type="button">
  ...
  $('#passwordForm',root).onsubmit=async e=>{e.preventDefault();const b=e.currentTarget.querySelector('button');loading(b,true);try{const d = await api('/api/user/profile-update',{
  method:'POST',
  body:JSON.stringify({
    action:'password',
    ...formDataObject(e.currentTarget)
...
if(d.logoutRequired){
  ...
  showAuth();
  toast('Şifren değiştirildi. Güvenlik için tekrar giriş yap.');
  return;
}

toast('Şifren değiştirildi.');}catch(err){toast(err.message,'error');}finally{loading(b,false,'Şifreyi Değiştir');}};
  $('#exportDataButton',root).onclick=()=>{window.location.href='/api/user/export-data';};
  $('#logoutAllDevicesButton',root).onclick=async()=>{
  if(!confirm('Tüm cihazlardaki oturumların kapatılacak. Devam etmek istiyor musun?'))return;
  ...
  try{
    ...
    showAuth();
    toast('Tüm cihazlardan çıkış yapıldı.');
